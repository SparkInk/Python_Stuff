#For python3
#This method includes complex numbers for a quadratic equation
import math
...
for i in P:
    print('Enter:', i)
    P[i] = int(input())
D = P['b'] ** 2 - 4 * P['a'] * P['c'] #Figure out a disriminant
if D > 0:
    ...
    print('D is', round(D, 3))
        ...
        x1 = complex() #Unfortunatly i couldn't round complex numbers, so it is what it is
        x2 = complex()
        Dc = D ** (1/2)
        x1 = (-P['b'] ** 2 + Dc) / (2 * P['a'])
        x2 = (-P['b'] ** 2 - Dc) / (2 * P['a'])
        print('As Discriminant D < 0, root will be complex')
        print('x1 ->', x1) #It will be long numbers
        print('x2 ->', x2)
        print('D is', Dc)
